...
public class ControllerGains{
    ...
    public boolean isProfiled() {
        return profileConstraints.maxAcceleration != Double.POSITIVE_INFINITY && profileConstraints.maxVelocity != Double.POSITIVE_INFINITY;
    }
    ...
    public void initSendable(SendableBuilder builder) {
        if(isProfiled()){
            ...
            builder.addDoubleProperty("maxVelocity", () -> profileConstraints.maxVelocity, (x) -> {
                profileConstraints = new TrapezoidProfile.Constraints(x, profileConstraints.maxAcceleration);
                controllerProfile = new TrapezoidProfile(profileConstraints);
            });

            builder.addDoubleProperty("maxAcceleration", () -> profileConstraints.maxAcceleration, (x) -> {
                profileConstraints = new TrapezoidProfile.Constraints(profileConstraints.maxVelocity, x);
    ...
    private void buildFeedForwardSendable(SendableBuilder builder) {
        builder.addDoubleProperty("simpleFeedForward", controllerFeedForwards::getSimpleFeedForward,
                (value) -> controllerFeedForwards.updateFeedForwards(value, ControllerFeedForwards.ChangeType.SIMPLE_FEED_FORWARD));

        builder.addDoubleProperty("frictionFeedForward", controllerFeedForwards::getFrictionFeedForward,
                (value) -> controllerFeedForwards.updateFeedForwards(value, ControllerFeedForwards.ChangeType.FRICTION_FEED_FORWARD));

        builder.addDoubleProperty("k_V", controllerFeedForwards::getK_V,
                (value) -> controllerFeedForwards.updateFeedForwards(value, ControllerFeedForwards.ChangeType.K_V));
    }

    /**
     * builds the sendable for the PID gains (k_P, k_I, k_D, i_Zone, i_MaxAccum, tolerance, maxOutput, minOutput)
     * @param builder the sendable builder
     */
    private void buildPIDSendable(SendableBuilder builder) {
        builder.addDoubleProperty("p", pidGains::getK_P,
                (value) -> updatePIDGains(value, PIDGains.ChangeType.K_P));

        builder.addDoubleProperty("i", pidGains::getK_I,
                (value) -> updatePIDGains(value, PIDGains.ChangeType.K_I));

        builder.addDoubleProperty("d", pidGains::getK_D,
                (value) -> updatePIDGains(value, PIDGains.ChangeType.K_D));

        builder.addDoubleProperty("izone", pidGains::getI_Zone,
                (value) -> updatePIDGains(value, PIDGains.ChangeType.I_ZONE));

        builder.addDoubleProperty("iMaxAccum", pidGains::getI_MaxAccum,
                (value) -> updatePIDGains(value, PIDGains.ChangeType.I_MAX_ACCUM));

        builder.addDoubleProperty("tolerance", pidGains::getTolerance,
                (value) -> updatePIDGains(value, PIDGains.ChangeType.TOLERANCE));

        builder.addDoubleProperty("maxOutput", pidGains::getMaxOutput,
                (value) -> updatePIDGains(value, PIDGains.ChangeType.MAX_OUTPUT));

        builder.addDoubleProperty("minOutput", pidGains::getMinOutput,
