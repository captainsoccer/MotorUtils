...
public class BasicSimMotor extends BasicSimSystem{
    ...
    public BasicSimMotor(BasicMotorConfig config) {
        this(createSimMotor(config), config.motorConfig.name, config.getControllerGains(), config.motorConfig.unitConversion);
    }
    ...
    private static DCMotorSim createSimMotor(BasicMotorConfig config){
        ...
        return new DCMotorSim(plant, config.motorConfig.motorType.withReduction(config.motorConfig.gearRatio), simConfig.positionStandardDeviation, simConfig.velocityStandardDeviation);
    }
