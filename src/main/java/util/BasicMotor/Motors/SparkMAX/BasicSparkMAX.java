...
public class BasicSparkMAX extends BasicMotor {
    ...
    public BasicSparkMAX(ControllerGains gains, int id, boolean isBrushless, String name, double gearRatio, MotorManager.ControllerLocation location) {

        super(gains, name, location);

        this.motor =
                new SparkMax(id, isBrushless ? SparkLowLevel.MotorType.kBrushless : SparkLowLevel.MotorType.kBrushed);

        this.config = new SparkMaxConfig();
        config.voltageCompensation(MotorManager.motorIdleVoltage); // set the voltage compensation to the idle voltage
    ...
    protected void setMotorOutput(double setpoint, double feedForward, Controller.RequestType mode) {
        ...
        switch(mode){
            ...
            case POSITION, PROFILED_POSITION -> setClosedLoopOutput(setpoint, feedForward, SparkBase.ControlType.kPosition);
            //set the motor output to a velocity
            case VELOCITY, PROFILED_VELOCITY -> setClosedLoopOutput(setpoint, feedForward, SparkBase.ControlType.kVelocity);
        }
    }
    ...
    public void setIdleMode(IdleMode mode) {
        SparkBaseConfig.IdleMode value = switch (mode) {
