...
public class PIDGains{
    ...
    /**
     * the tolerance of the PID controller (if the error is less than this value, the controller will stop)
     */
     ...
     * @param i_maxAccum the maximum accumulation of the integrator (>= 0) this is the maximum value that the integrator can accumulate
     */
    public PIDGains(double k_P, double k_I, double k_D, double i_Zone, double i_maxAccum, double tolerance, double maxOutput, double minOutput) throws IllegalArgumentException{
        ...
        if(maxOutput <= minOutput) throw new IllegalArgumentException("maxOutput must be greater than minOutput");

        if(maxOutput < -maxMotorOutput) throw new IllegalArgumentException("maxOutput must be greater than " + -maxMotorOutput);
        if(maxOutput > maxMotorOutput) throw new IllegalArgumentException("maxOutput must be less than " + maxMotorOutput);
        this.maxOutput = maxOutput;

        if(minOutput < -maxMotorOutput) throw new IllegalArgumentException("minOutput must be greater than " + -maxMotorOutput);
    ...
    public PIDGains(double k_P, double k_I, double k_D) throws IllegalArgumentException{
        this(k_P, k_I, k_D, Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY, 0, maxMotorOutput, -maxMotorOutput);
    }
     ...
     * @param k_D the derivative gain (>= 0)
     */
    public PIDGains(double k_P, double k_I, double k_D, double tolerance) throws IllegalArgumentException{
        this(k_P, k_I, k_D, Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY, tolerance, maxMotorOutput, -maxMotorOutput);
    }
     ...
     * @return true if the value has changed, false if it has not (used for setting the built-in controller values)
     */
