 ...
 * current draw of the motor to prevent overheating and damage to the motor.
 * this is modeled after the TalonFX current limits
 * so other motor controllers may not have the same limits
 */
public class CurrentLimits {
    /**
     * the maximum current output of the motor controller (in amps)
     * this is different from supplyCurrentLimit
     */
    private final double statorCurrentLimit;
    /**
     * the maximum current draw of the motor controller (in amps)
     * this is different from statorCurrentLimit
     */
    private final double supplyCurrentLimit;
    /**
     * the time (in seconds) that the motor can stay at supply current limit
     * before it lowers to supply lower limit
     ...
     * @param supplyLowerLimit   the current the motor drops to after the supply current limit is reached for
     */
    ...
    /**
     * gets the time (in seconds) that the motor can stay at supply current limit
     * before it lowers to supply lower limit
     * this is useful for bursts of current
     ...
     * this is useful for bursts of current
     * @return the current the motor drops to after the supply current limit is reached for supplyLowerTime
     */
