 ...
 * the rev motor controllers ignore the supply current limit and supply lower time
 * if you don't need to use the free speed RPM or stall current limit, you can use the normal CurrentLimits class
 */
public class CurrentLimitsREV extends CurrentLimits{
     ...
     *
     * @param freeSpeedCurrentLimit the maximum current output of the motor controller (in amps) when not in stall
     * @param stallCurrentLimit the maximum current output of the motor controller (in amps) while in stall
     * @param freeSpeedRPS the free speed of the motor controller (in RPS (revolutions per second))
